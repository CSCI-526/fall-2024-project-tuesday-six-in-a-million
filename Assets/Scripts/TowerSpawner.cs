...
public class TowerSpawner : MonoBehaviour
{
    ...
    {
        Debug.Log("SpawnMortar");
        int goldCost = MortarPrefab.GetComponent<Mortar>().goldCost;
        if (!goldUpdater.SubtractGold(goldCost))
        {
            alertText.text = "Not enough gold to spawn Mortar";
            return;
        }
        GameObject tower = Instantiate(MortarPrefab, transform.position + new Vector3(2, 0, 0), transform.rotation);
        tower.GetComponent<Mortar>().goldUpdater = goldUpdater;
        tower.SetActive(true);
        Debug.Log("Light tower spawned");
    }
